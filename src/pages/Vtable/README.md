...
组织与科目的收展规则和常规模式相同：组织仅控制子组织的显示，科目仅控制本组织内该科目的明细。区域组织的 10 个科目各有 100 条明细，100 个中心组织的 10 个科目各有 90 条明细，共 100,000 条。全部 1,100 个科目均可独立收展；收起组织不影响它自身的科目，重新展开组织会保留子组织原有的科目状态。

1. `project` 只返回视图 ID、总行数和层级元数据。
2. 首次及滚动后的 `page` 请求每批最多 **200 行**，预取当前视口之后的一批。
3. 页请求去重，最多 **3 个并发请求**；显式读取、当前视口、下一页预取依次优先。快速滚动取消过期预取，切换模式/层级取消旧请求。LRU 最多 **10 页 / 2,000 行**，丢弃迟到响应。失败按页保留，其他成功页不会触发自动重试；点击“重试当前页”恢复。
...
- `../TanStackBudget/core/page-cache.ts`：独立的分页调度器，负责并发、优先级、取消、LRU、响应完整性和按页失败，不依赖 React 或表格引擎。
- `../TanStackBudget/core/use-cell-annotations.ts`：批注与附件的会话状态、校验及 Object URL 释放。
...
### 十万模式与常规模式一致的独立收展（2026-09-09）

- 修复区域级科目没有明细、缺少折叠箭头的问题。区域科目各 100 条明细、中心科目各 90 条明细，仍为 100,000 条明细与 1,100 条汇总；直接复用常规模式的投影和切换逻辑，不引入父子科目联动。
- 组织只控制子组织，科目只控制自身明细。收起组织后，其自身科目仍可操作；重新展开组织保留子组织内各科目的收展状态。汇总与明细仍独立保存。
- 为组织列补齐 `textStickBaseOnAlign`。VTable 的滚动文字布局读取列样式，单独在合并样式中设置不能让长合并区文字保持可见；现在跨页滚动和科目收展后，名称与箭头都位于合并区可见部分的中央。
- 逻辑回归逐项检查全部 1,100 个科目，并用相同流程验证常规与十万模式的组织、科目、下钻和数据恢复。新增 `hierarchy.spec.ts` 验证区域首、中、尾科目跨页反复收展、合并跨度、文字实际绘制位置、父子状态独立及缓存上限。原尾页、远处折叠用例随明细分布更新坐标，保留原有断言强度。

本轮验收：VTable 类型检查与格式检查、12 项业务逻辑测试、5 项选区测试通过。数据修复后的完整 VTable 浏览器回归 **53/53** 通过；补齐组织文字滚动配置后，最终相关专项 **20/20** 通过，共用 TanStack 入口 **15/15** 通过。完整回归报告保留在 `playwright-report/vtable-hierarchy-full/`，最终专项报告位于 `playwright-report/vtable-budget/`。已目视检查 `test-results/vtable-hierarchy-region-1.png`，确认跨页组织名称和折叠箭头可见。本轮使用 8000 端口开发服务验收，未重新执行生产构建；服务已重启使新的压力数据分布生效。


### 页面交付整理（2026-09-09）

- 分页调度与 React 生命周期解耦，两个表格入口共用有界请求队列。取消过期滚动请求，校验页码、投影 ID 及返回行数；失败不会因为其他页成功而循环重试。
- 批注和附件有独立状态模块。修改历史仅在历史侧栏打开时计算，并缓存当前单元格结果。批量变更通知分帧处理，保留逐格回调顺序。
- 编辑后打开侧栏统一先提交一次；校验失败保留原输入与焦点。超限粘贴在解析时即拒绝，不构造超大矩阵或触发写请求。列宽适配沿用滚动锚点，保留折叠底部的展示留白。
- 演示网关通过 `close` 在路由离开/页面退出时释放服务端会话，避免反复进入耗尽 100 个会话名额；异常退出仍由原有过期清理兜底。React StrictMode 重放与浏览器往返缓存不会提前释放活跃会话。
- `BudgetWorkbench` 增加可选 `gateway` 注入入口。当前仓库交付继续使用演示服务，金额、批注和附件刷新重置；正式网关可以不实现演示用 `dispose`。网关实例需在工作台挂载前创建，切换数据源时重新挂载工作台。

新增回归：`page-cache.test.ts` 覆盖队列与缓存竞态、分页校验、超限粘贴及 120 次会话释放；`reliability.spec.ts` 覆盖浏览器中的编辑切换、部分分页失败、慢网快速滚动、大量粘贴和离页清理。最终验收与性能记录见 [交付记录](./DELIVERY.md)。

参考依据：[VTable 异步分页](https://visactor.io/vtable/guide/data/async_data)、[配置更新与布局开销](https://visactor.io/vtable/guide/basic_function/update_option)；实现同时核对本项目锁定的 1.26.7 源码。

本轮最终验收：生产构建、三套类型检查、格式检查通过；逻辑测试 26 / 26，VTable 生产浏览器回归 58 / 58，共用 TanStack 生产回归 15 / 15。十万行本机进入 612ms、尾页保存 271ms、完整选区统计 800ms，流程仅有 4 个分页响应。开发预览已恢复至 8000 端口。详细条件、首次测试取样修正及截图见交付记录。
