...
| 产品属性 | 后台产品节点的 `productAttribute` | 跟随产品块纵向合并，可编辑且不参与折叠 |
...
列树与 `BUSINESS_DATA` 通过 `field` 配合：业务叶子列的 `field` 必须是业务节点上的字段，例如 `revenue` 和 `productAttribute`；只有产品层级和区域层级是纯投影字段。应用启动时会检查所有业务节点 `id` 和完整行维唯一，并检查每个业务节点是否具有后台列配置要求的字段，配置错误会立即报出。
...
- 更新日期选择器；
- 调整系数数值校验；
- 每个业务字段独立编辑，不在前端重算或联动其他字段；
- 字段是否可编辑完全由后台列配置的 `editable` 决定；
- 产品层级和区域层级不可编辑；产品属性及后台允许编辑的汇总、明细字段都可编辑。

数据区遵循统一视觉规则：后台 `hierarchyRole` 为 `category`、`subcategory` 或 `region` 的汇总记录使用 600 字重，`detail` 明细记录使用 400 字重；复选框和日期居中；数据区隐藏横向网格线，避免空白控件单元格出现类似下划线的连续短线。

`getCellEditability()` 是唯一的可编辑策略入口。只要后台列配置允许编辑，并且当前投影单元格能够唯一映射到 `BUSINESS_DATA` 中的一条后台记录，就允许编辑；这既包括叶子明细，也包括产品、子类和区域汇总。产品属性映射当前产品块的 category / subcategory 产品节点，其余业务字段映射当前区域或明细节点。纯层级字段、后台标记为只读的字段或无法唯一映射的投影数据保持只读。控制器同时使用 SpreadJS 单元格锁定、`EditStarting` 和粘贴前校验执行这套规则，不能只依赖样式。

`BUSINESS_DATA` 是完整的后台业务树，必须包含所有汇总数据：产品汇总、区域汇总和叶子明细都各自拥有稳定 `id` 与完整指标，category / subcategory 产品节点同时直接携带 `productAttribute`。前端不会根据明细重新聚合或覆盖汇总值；编辑产品属性或汇总指标都只更新对应节点，不会自动向下分摊到明细。
...
从两棵后台树生成稳定的行维路径和列维路径
  ↓ updateBusinessNode()
只更新当前直接字段，不修改任何其他业务字段
  ↓ createBusinessProjectionRows() / createStressProjectionRows()
常规模式直接更新 BUSINESS_DATA 节点并重新投影；压力模式重新生成当前投影
  ↓ 比较修改前后的 ViewRow[]
刷新直接修改及同一后台记录的可见投影，并写入历史
```

具体规则如下：

- 修改净收入、商品收入、服务收入、订单数、线上订单、线下订单或客单价时，只更新被编辑的字段；
- 修改核验状态时不再同步“已核验”，修改“已核验”也不再同步核验状态；
- 前端不根据其他字段计算客单价、目标达成等指标；
- 常规模式的产品和区域汇总指标由后端直接返回，并保存在对应的 `BUSINESS_DATA` 树节点上。编辑明细只改变该明细节点，编辑汇总也只改变对应汇总节点；刷新与最终值以后端响应为准。
...
如果同一后台记录同时出现在多个可见投影位置，页面会同步这些镜像单元格并标记为“投影同步”；这不是业务字段联动。区域、产品及事业群等汇总行不随明细编辑变化。
...
- **数据追踪**：展示当前值对应的后台字段与业务记录 ID，并明确前端不计算、不联动；
- **历史**：按稳定单元格 ID 展示所有值变化。

当前附件和历史都是 Demo 级浏览器内存实现，刷新后不会持久化。数据追踪展示的是对接协议示意，正式环境应替换为后台返回的真实血缘信息。
...
| `EditStarting` | 按后台列配置阻止只读或无法唯一映射的单元格编辑 |
...
`COLUMNS`、多级表头与列 Outline 都由 `BUSINESS_COLUMN_DATA` 派生，不要直接改派生数组。新增列后仍要检查依赖明确列号的格式化和验证逻辑。
...
5. `commitBusinessCellValues()`：提交值并确认只有目标字段发生变化；
...
- 搜索能命中折叠内容并只展开必要祖先；
- 编辑、粘贴、清空、撤销、重做都有历史；
- 能唯一映射到后台记录、且后台列配置 `editable: true` 的汇总与明细指标可以编辑；层级、属性和歧义投影保持只读；
- 修改收入、订单、客单价、状态或“已核验”时，其他字段保持不变；
- 修改城市明细后，后端汇总静态值保持不变；
