...
常规模式的行折叠是“重新投影并渲染数据”，不是 SpreadJS 原生行 Outline。10 万行模式为了性能，改用两级原生行 Outline：

- level 0：产品层级；
- level 1：区域层级。

两种模式最终都呈现相同的产品块、属性列和区域列布局。
...
10 万行模式使用表内 Outline 展开层级，不提供页面级下钻，避免同时维护两套导航语义。
...
10 万行模式直接搜索内存中的 `ViewRow`。扫描过程中每 5,000 行让出一次主线程，使搜索可以取消，并避免长时间冻结页面。
...
压力模式生成固定的 100,000 个 `ViewRow`，数据层级为：
...
- Worksheet 保持固定物理行数，折叠时只切换 Outline 可见性；
- 每页物化 400 行；
...
常规模式调用 `renderProjectionRows()` 重新投影；压力模式只物化当前需要的页。
...
压力模式还需同步检查 `getStressRowOutlineGroups()`、Outline level、可见行统计和懒加载范围。
