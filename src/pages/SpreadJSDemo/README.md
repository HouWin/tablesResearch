...
- 层级列、产品属性以及后台标记为只读的字段不可编辑；后台返回的汇总记录与明细记录都可编辑。
...
`getCellEditability()` 是唯一的可编辑策略入口。只要后台列配置允许编辑，并且当前投影单元格能够唯一映射到 `BUSINESS_DATA` 中的一条后台记录，就允许编辑；这既包括叶子明细，也包括产品、子类和区域汇总。层级/属性字段、后台标记为只读的字段或无法唯一映射的投影数据保持只读。控制器同时使用 SpreadJS 单元格锁定、`EditStarting` 和粘贴前校验执行这套规则，不能只依赖样式。
