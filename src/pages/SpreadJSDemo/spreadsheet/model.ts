...
export type PanelName = "comment" | "history" | "lineage" | "attachment" | "aggregate" | "features" | null;
export type AggregateMode = "SUM" | "AVG" | "COUNT" | "MIN" | "MAX" | "CUSTOM";
export type DataMode = "regular" | "loading" | "stress";
export type ToastTone = "success" | "error";
export type NumericDisplay = "currency" | "percent" | "decimal" | "number" | "mixed";
...
export type HierarchyField = "categoryHierarchy" | "subcategoryHierarchy" | "regionHierarchy" | "detailHierarchy";
export type BusinessField = Exclude<keyof BusinessNode, "id" | "children" | "hierarchyRole">;
...
export const AGGREGATE_MODES = ["SUM", "AVG", "COUNT", "MIN", "MAX", "CUSTOM"] as const satisfies readonly AggregateMode[];
...
export const STRESS_TEXT_SEARCH_COLUMNS = new Set([0, 1, 2, 3, 12, 13, 14, 15, 16]);
...
function makeGroup(
  ...
  const completion = children.reduce((sum, child) => sum + child.completion, 0) / Math.max(children.length, 1);
  return makeNode(id, name, hierarchyRole, revenue, orders, completion, owner, status, "2026-08-21", children);
}

function makeRegion(id: string, name: string, children: BusinessNode[], owner: string) {
  return makeGroup(id, name, "region", children, owner);
}

const furnitureBookcases = makeGroup("furniture-bookcases", "书柜", "subcategory", [
  makeRegion("bookcases-east", "华东", [
    makeNode("bookcases-shanghai", "上海", "detail", 2_086_400, 352, 0.982, "杨晨", "已核验", "2026-08-21"),
    makeNode("bookcases-jiangsu", "江苏", "detail", 1_638_400, 294, 0.953, "陈叶", "待复核", "2026-08-21"),
  ], "周宁"),
  makeRegion("bookcases-central", "华中", [
    makeNode("bookcases-hubei", "湖北", "detail", 1_286_600, 238, 0.942, "孙毅", "已核验", "2026-08-21"),
    makeNode("bookcases-henan", "河南", "detail", 1_006_600, 206, 0.899, "徐昕", "待复核", "2026-08-20"),
  ], "赵敏"),
], "林嘉");

const furnitureChairs = makeGroup("furniture-chairs", "座椅", "subcategory", [
  makeRegion("chairs-east", "华东", [
    makeNode("chairs-zhejiang", "浙江", "detail", 2_483_500, 414, 0.934, "吴哲", "已核验", "2026-08-20"),
    makeNode("chairs-anhui", "安徽", "detail", 1_783_500, 314, 0.904, "韩睿", "已核验", "2026-08-20"),
  ], "周宁"),
  makeRegion("chairs-south", "华南", [
    makeNode("chairs-guangdong", "广东", "detail", 3_286_400, 596, 0.928, "黄清", "待复核", "2026-08-21"),
    makeNode("chairs-fujian", "福建", "detail", 1_527_800, 322, 0.881, "罗蔚", "已核验", "2026-08-20"),
  ], "苏然"),
], "林嘉");

const officePaper = makeGroup("office-paper", "纸品", "subcategory", [
  makeRegion("paper-east", "华东", [
    makeNode("paper-shanghai", "上海", "detail", 1_486_400, 442, 0.972, "杨晨", "已核验", "2026-08-21"),
    makeNode("paper-nanjing", "南京", "detail", 1_138_400, 344, 0.943, "陈叶", "待复核", "2026-08-21"),
  ], "周宁"),
  makeRegion("paper-north", "华北", [
    makeNode("paper-beijing", "北京", "detail", 1_686_600, 408, 0.922, "孙毅", "已核验", "2026-08-21"),
    makeNode("paper-tianjin", "天津", "detail", 906_600, 256, 0.889, "徐昕", "待复核", "2026-08-20"),
  ], "赵敏"),
], "罗蔚");

const officeStorage = makeGroup("office-storage", "收纳", "subcategory", [
  makeRegion("storage-central", "华中", [
    makeNode("storage-wuhan", "武汉", "detail", 1_583_500, 374, 0.924, "吴哲", "已核验", "2026-08-20"),
    makeNode("storage-changsha", "长沙", "detail", 1_183_500, 284, 0.894, "韩睿", "已核验", "2026-08-20"),
  ], "周宁"),
  makeRegion("storage-south", "华南", [
    makeNode("storage-shenzhen", "深圳", "detail", 2_186_400, 496, 0.918, "黄清", "待复核", "2026-08-21"),
    makeNode("storage-xiamen", "厦门", "detail", 1_227_800, 302, 0.871, "罗蔚", "已核验", "2026-08-20"),
  ], "苏然"),
], "罗蔚");

export const BUSINESS_DATA = [
  makeGroup("furniture", "家具", "category", [furnitureBookcases, furnitureChairs], "林嘉"),
  makeGroup("office-supplies", "办公用品", "category", [officePaper, officeStorage], "罗蔚", "待复核"),
];
...
export const INITIAL_DATASET_LABEL = `${flattenTree(BUSINESS_DATA).length} 行 × ${COLUMNS.length} 列`;

export function findBusinessNode(nodes: BusinessNode[], nodeId: string): BusinessNode | undefined {
...
export function rootsForView(view: DrillView, roots: BusinessNode[] = BUSINESS_DATA) {
...
export function canDrillNode(node: BusinessNode | ViewRow | null | undefined) {
  return Boolean(node?.children?.length || node && "hasChildren" in node && node.hasChildren);
}

export function viewForNode(view: DrillView, node: BusinessNode | ViewRow): DrillView | null {
...
export function flatRowsForView(rows: ViewRow[], view: DrillView): ViewRow[] {
  ...
  let parentLevel = -1;
  for (const pathItem of view) {
    const nodeIndex = rows.findIndex((row, index) => (
      index >= rangeStart
      && index < rangeEnd
    ...
    rangeEnd = rangeStart;
    while (rangeEnd < rows.length && rows[rangeEnd].level > parentLevel) rangeEnd += 1;
  }
...
const STRESS_OWNERS = ["林嘉", "周宁", "杨晨", "陈叶", "赵敏", "孙毅", "徐昕", "吴哲"];
const STRESS_CITIES = ["上海", "苏州", "南京", "杭州", "广州", "厦门", "北京", "天津"];
...
function createStressRecord(index: number): ViewRow {
  ...
  const status: Status = index % 17 === 0 ? "异常" : index % 5 === 0 ? "待复核" : "已核验";
  const name = isRegion
...
export async function getStressRecordsAsync() {
  ...
  stressRecordsPromise ??= (async () => {
    ...
    for (let start = 0; start < rows.length; start += chunkSize) {
      ...
      for (let index = start; index < end; index += 1) rows[index] = createStressRecord(index);
      if (end < rows.length) {
...
export function getRowOutlineGroups(rows: ViewRow[]): RowOutlineGroup[] {
  ...
  rows.forEach((row, index) => {
    ...
    if (row.hasChildren || row.children?.length) open.push({ summaryRow: index, level: row.level });
  });
...
export function isHierarchyField(field: ColumnField): field is HierarchyField {
  return field === "categoryHierarchy"
...
export function viewRowValues(row: ViewRow, columnCount: number) {
  return Array.from({ length: columnCount }, (_, col) => viewRowCellValue(row, col));
}

export function stressCellSearchText(row: ViewRow, col: number, includeFormattedNumber: boolean) {
  ...
  if (typeof value === "number") {
    if (col === COMPLETION_COLUMN) return `${value} ${(value * 100).toFixed(1)}%`;
    if (col === DECIMAL_COLUMN) return `${value} ${value.toFixed(2)}`;
    if (includeFormattedNumber) {
      const currency = col >= REVENUE_COLUMN && col <= SERVICE_REVENUE_COLUMN || col === AVG_ORDER_COLUMN ? "¥" : "";
...
export function updateBusinessNode(node: BusinessNode, field: BusinessField, value: unknown) {
  switch (field) {
    case "name":
    ...
    case "attachment":
      ...
      break;
    default:
      if (typeof value === "number" && Number.isFinite(value)) node[field] = value;
  }
}
...
export function numericDisplayForColumn(col: number): Exclude<NumericDisplay, "mixed"> {
  if (col >= REVENUE_COLUMN && col <= SERVICE_REVENUE_COLUMN || col === AVG_ORDER_COLUMN) return "currency";
...
export function formatStatistic(value: number, display: NumericDisplay) {
  if (display === "currency") return `¥${Math.round(value).toLocaleString("zh-CN")}`;
...
export function getAggregateValue(stats: SelectionStats, mode: AggregateMode, customFormula: string) {
