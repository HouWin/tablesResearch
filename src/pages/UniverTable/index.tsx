 ...
 *
 * 演示 ETable 组件的能力：
 * - 费用预算表（默认）：FormSchema（columns/records）→ adaptFormSchemaToETable
 * - 费用预算 1万～100万行压测（generateScaledExpenseBudgetTreeData）
 * - 组织科目 1万～100万行压测（generateScaledExpenseBudgetOrgTreeData）
...
import { generateScaledExpenseBudgetOrgTreeData } from './expenseBudgetOrgDataGenerator';
import {
...
/** 数据规模：表单预算 / 组织科目预算 / 预算压测 / 经营树 */
const DATA_SCALE_OPTIONS = [
  { value: 'budget', label: '费用预算（表单 schema）' },
  { value: 'budget-org', label: '费用预算（组织科目）' },
  ...BUDGET_SCALE_ROWS.map((rows) => ({
    value: `budget-org-${rows}` as const,
    label: `组织科目 ${rows / 10000}万行`,
  })),
  ...BUDGET_SCALE_ROWS.map((rows) => ({
...
const parseBudgetScaleRows = (value: DataScale): number | null => {
  ...
  // 排除 budget-org / budget-org-N（组织科目）
  if (value === 'budget-org' || value.startsWith('budget-org-')) {
    return null;
  }
...
const parseBudgetOrgScaleRows = (value: DataScale): number | null => {
  if (typeof value !== 'string' || !value.startsWith('budget-org-')) {
    return null;
  }
  const rows = Number(value.slice('budget-org-'.length));
  return Number.isFinite(rows) && rows > 0 ? rows : null;
};
...
const UniverTablePage = () => {
  ...
  const budgetDemoAdapted = useMemo(
    () => adaptFormSchemaToETable(expenseBudgetFormData),
    [],
  );
  const [budgetScaledMeta, setBudgetScaledMeta] = useState<
    ReturnType<typeof adaptFormSchemaToETable>['meta'] | null
  >(null);
  const [budgetScaledConfig, setBudgetScaledConfig] = useState<
    ETableTreeConfig | null
  >(null);

  const budgetScaleRows = parseBudgetScaleRows(dataScale);
  const budgetOrgScaleRows = parseBudgetOrgScaleRows(dataScale);
  const isBudgetForm = dataScale === 'budget';
  const isBudgetOrgSample = dataScale === 'budget-org';
  const isBudgetOrg = isBudgetOrgSample || budgetOrgScaleRows != null;
  const isBudgetDemo = isBudgetForm || isBudgetOrgSample;
  const isBudgetMode =
    isBudgetDemo || budgetScaleRows != null || budgetOrgScaleRows != null;
  const isDemoTree = dataScale === 'tree';
  const targetRowCount =
    typeof dataScale === 'number'
      ? dataScale
      : budgetOrgScaleRows ?? budgetScaleRows ?? 0;

  const budgetMeta = isBudgetForm
    ? budgetDemoAdapted.meta
    : budgetScaledMeta ?? budgetDemoAdapted.meta;

  // ---------- 派生数据：费用预算树 / 演示树 / 生成树 + 动态追加列 ----------
  const activeTreeData = useMemo(() => {
    if (isBudgetOrgSample) {
      return expenseBudgetTreeData;
    }
    if (budgetOrgScaleRows != null) {
      return scaledTreeData ?? [];
    }
    if (isBudgetForm) {
      return budgetDemoAdapted.treeData;
    }
    if (isBudgetMode) {
      return scaledTreeData ?? [];
    }
  ...
  const activeTreeConfig = useMemo(() => {
    ...
    if (isBudgetMode) {
      if (isBudgetForm) {
        return budgetDemoAdapted.treeConfig;
      }
      return (
        budgetScaledConfig ?? {
          ...budgetDemoAdapted.treeConfig,
          liteMode: true,
          skipMerges: true,
          defaultCollapsed: true,
        }
  ...
  const loadScaledBudgetTree = useCallback(async (count: number) => {
    ...
    try {
      ...
      setScaledTreeData(generated);
      setBudgetScaledConfig(treeConfig);
      setBudgetScaledMeta(meta);
      setFlatRowCount(rows);
      setTracks([]);
      setTableKey((key) => key + 1);
      message.success(
        `成功生成费用预算树，约 ${orgCount.toLocaleString()} 个科目节点 / ${rows.toLocaleString()} 行（展平后）`,
      );
    } catch {
  ...
  const loadScaledBudgetOrgTree = useCallback(async (count: number) => {
    if (count >= 500000) {
      message.warning('数据量较大，生成与渲染可能较慢，请耐心等待');
    }
    setLoading(true);
    ...
    try {
      const {
        ...
        flatRowCount: rows,
        orgCount,
      } = await generateScaledExpenseBudgetOrgTreeData(count, setProgress);
      setScaledTreeData(generated);
      setBudgetScaledConfig(null);
      setBudgetScaledMeta(null);
      setFlatRowCount(rows);
      setTracks([]);
      setTableKey((key) => key + 1);
      message.success(
        `成功生成组织科目树，约 ${orgCount.toLocaleString()} 个组织 / ${rows.toLocaleString()} 行（与示例同构）`,
      );
    } catch {
      message.error('组织科目数据生成失败');
      setTableRendering(false);
    } finally {
      setLoading(false);
      setProgress(100);
  ...
  const handleScaleChange = async (value: DataScale) => {
    ...
    const nextBudgetOrgRows = parseBudgetOrgScaleRows(value);
    if (
      typeof value === 'number' ||
      nextBudgetRows != null ||
      nextBudgetOrgRows != null
    ) {
      setLoading(true);
      setTableRendering(true);
    }
    ...
    if (value === 'budget') {
      ...
      setTableRendering(true);
      setTableKey((key) => key + 1);
      message.success('已切换到费用预算（表单 schema）示例');
      return;
    }
    if (value === 'budget-org') {
      setScaledTreeData(null);
      ...
      setTableRendering(true);
      setTableKey((key) => key + 1);
      message.success('已切换到费用预算（组织 / 科目）示例');
      return;
    }
    if (nextBudgetOrgRows != null) {
      await loadScaledBudgetOrgTree(nextBudgetOrgRows);
      return;
    }
  ...
  const handleRegenerate = async () => {
    if (dataScale === 'budget' || dataScale === 'budget-org') {
      setTracks([]);
      setRenderMs(null);
      setTableRendering(true);
      setTableKey((key) => key + 1);
      message.success(
        dataScale === 'budget-org'
          ? '已重新加载费用预算（组织科目）示例'
          : '已重新加载费用预算（表单 schema）示例',
      );
      return;
    }
    const budgetOrgRows = parseBudgetOrgScaleRows(dataScale);
    if (budgetOrgRows != null) {
      await loadScaledBudgetOrgTree(budgetOrgRows);
      return;
    }
  ...
  const stats = useMemo(() => {
    if (isBudgetOrg) {
      ...
      return {
        ...
        modeLabel:
          budgetOrgScaleRows != null ? '组织科目大数据' : '组织科目预算',
      };
    }
    if (isBudgetMode) {
      const sheetRows = isBudgetForm
        ? sheetRowCount || budgetMeta.flatRowCount
        : flatRowCount || sheetRowCount || 0;
      const totalCols =
        budgetMeta.freezeCols + budgetMeta.valueFields.length;
      return {
        treeNodes: isBudgetForm
          ? budgetMeta.orgCount
          : countNodes(activeTreeData),
        sheetRows,
        totalCols,
        totalCells: sheetRows * totalCols,
        modeLabel: isBudgetForm ? '表单费用预算' : '费用预算大数据',
      };
    }
  ...
  const options = useMemo(() => {
    const headerDepth = isBudgetOrg
      ? EXPENSE_BUDGET_HEADER_DEPTH
      : isBudgetMode
        ? budgetMeta.headerDepth
        : HEADER_DEPTH;
    const freezeCols = isBudgetOrg
      ? EXPENSE_BUDGET_FREEZE_COLS
      : isBudgetMode
        ? budgetMeta.freezeCols
        : HIERARCHY_COLS;
    return {
      ...defaultOptions,
      name: isBudgetOrg
        ? budgetOrgScaleRows != null
          ? `费用预算表-组织科目 ${targetRowCount}`
          : '费用预算表-组织科目示例'
        : isBudgetForm
          ? '费用预算表-表单schema示例'
          : isBudgetMode
      ...
      <div style={{ padding: 24, background: '#f0f2f5', minHeight: '100%' }}>
            ...
            <Col>
              <h2 style={{ margin: 0 }}>
                {isBudgetMode
                  ? isBudgetOrg
                    ? 'Univer 费用预算表 / 组织科目'
                    : 'Univer 费用预算表 / 表单 schema'
                  : 'Univer 树形分组 / 大数据示例'}
              </h2>
              <p style={{ margin: '4px 0 0 0', color: '#666' }}>
                {isBudgetOrg
                  ? budgetOrgScaleRows != null
                    ? `组织树折叠 · 科目费用汇总 · 功能属性 · 支持 1万～100万行压测（当前约 ${targetRowCount.toLocaleString()} 行）`
                    : '组织树折叠 · 科目费用汇总 · 功能属性 · 全年合计 · 2025年1–12月 · treeUI ▶/▼ · 可选 1万～100万行压测'
                  : isBudgetMode
                    ? '预算科目树 · 功能属性 · 实际数/预算数多级表头 · FormSchema → ETable · treeUI ▶/▼ · 支持 1万～100万行压测'
                    : '三层表头 · 品类/区域树 · 上钻下钻 · 回撤 · 单元格历史 · 净收入 / 核验状态 / 日期'}
              </p>
            </Col>
            <Col>
              <Space wrap>
        ...
        <Card style={{ marginBottom: 16 }} styles={{ body: { padding: '12px 24px' } }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            >
              <Statistic title={isBudgetOrg ? '组织单元' : isBudgetMode ? '科目节点' : '树节点数'} value={stats.treeNodes} suffix="个" />
          ...
          <div style={{ flex: 1, minWidth: 0, transition: 'flex 0.28s ease' }}>
            <Card style={{ height: '100%' }}>
              ...
              <Alert
                ...
                description={
                  isBudgetOrg
                    ? '组织列同列缩进折叠（▶/▼）；科目列可展开费用汇总 / 日常费用合计明细。功能属性 / 全年合计 / 2025年月度可编辑；汇总行功能属性只读。'
                    : isBudgetForm
                      ? '预算科目列同列缩进折叠（▶/▼）；功能属性为枚举；实际数(2026)/预算数(2027) 多级表头。数据来自 FormSchema（columns/records），经 adaptFormSchemaToETable 接入。'
                      : isBudgetMode
                        ? `费用预算压测约 ${stats.sheetRows.toLocaleString()} 行（FormSchema records 树）。≥5000 行启用视口投影：左侧序号为展开后的行号，滚到底部自动加载下一页。${viewportRange ? ` ${viewportRange}` : ''}`
                        : isDemoTree
              ...
              {loading ? (
                ...
                */}
                  <ETable
                    ref={tableRef}
                    key={`${dataScale}-${tableKey}-${gridLines}-${freezeHeader}-${contextMenu}-${virtualScroll}-budget-dual-v1-${addedColumns.map((item) => item.id).join(',')}`}
                      ...
                      <li key={item.id} style={{ marginBottom: 8 }}>
                        ...
                        {(item.rowDimensions?.length || item.columnDimensions?.length) ? (
                          <div style={{ fontSize: 12, color: '#8c8c8c', marginTop: 2 }}>
                            {item.rowId
                              ? `rowId=${JSON.stringify(item.rowId)}; `
                              : ''}
                            {item.columnId || item.field
                              ? `columnId=${JSON.stringify(item.columnId ?? item.field)}; `
                              : ''}
          ...
          <Row gutter={[24, 20]}>
            <Col xs={24} lg={12}>
              ...
              <ul style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '6px 0 0', margin: 0, listStyle: 'none' }}>
                ...
                <li><Tag color="green">数据规模切换：表单预算 / 组织科目 / 组织科目1万～100万行 / 费用预算1万～100万行 / 树形演示 / 经营树1万～100万行；支持重新生成</Tag></li>
              </ul>
              ...
              <ul style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '6px 0 0', margin: 0, listStyle: 'none' }}>
                ...
                <li><Tag color="green">右键菜单：复制粘贴、批注、附件、历史、追踪、下钻、搜索、撤销</Tag></li>
              </ul>
            </Col>
            ...
            <Col xs={24} lg={12}>
              ...
              <ul style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '6px 0 0', margin: 0, listStyle: 'none' }}>
                ...
                <li><Tag color="cyan">搜索：openSearch / search；编辑：undo</Tag></li>
