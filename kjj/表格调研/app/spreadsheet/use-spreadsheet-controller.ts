...
type Workbook = import("@grapecity-software/spread-sheets").Spread.Sheets.Workbook;
type Worksheet = import("@grapecity-software/spread-sheets").Spread.Sheets.Worksheet;
type CellChangedArgs = import("@grapecity-software/spread-sheets").Spread.Sheets.ICellChangedEventArgs;
type CellClickArgs = import("@grapecity-software/spread-sheets").Spread.Sheets.ICellClickEventArgs;
type CellDoubleClickArgs = import("@grapecity-software/spread-sheets").Spread.Sheets.ICellDoubleClickEventArgs;
type SelectionChangedArgs = import("@grapecity-software/spread-sheets").Spread.Sheets.ISelectionChangedEventArgs;
type RangeGroupStateChangedArgs = import("@grapecity-software/spread-sheets").Spread.Sheets.IRangeGroupStateChangedEventArgs;
type TopRowChangedArgs = import("@grapecity-software/spread-sheets").Spread.Sheets.ITopRowChangedEventArgs;
type ValidationErrorArgs = import("@grapecity-software/spread-sheets").Spread.Sheets.IValidationErrorEventArgs;
type ClipboardChangedArgs = import("@grapecity-software/spread-sheets").Spread.Sheets.IClipboardChangedEventArgs;
type ClipboardPastingArgs = import("@grapecity-software/spread-sheets").Spread.Sheets.IClipboardPastingEventArgs;
...
export function useSpreadsheetController() {
  ...
  const historyRef = useRef<Map<string, HistoryItem[]>>(new Map([
    [stableCellKey("bookcases-shanghai", "revenue"), [
      { id: "history-3", oldValue: 2_062_000, newValue: 2_086_400, source: "日报回写", createdAt: new Date("2026-08-21T16:34:00+08:00").getTime() },
      { id: "history-2", oldValue: 2_048_400, newValue: 2_062_000, source: "批量粘贴", createdAt: new Date("2026-08-21T14:18:00+08:00").getTime() },
      { id: "history-1", oldValue: null, newValue: 2_048_400, source: "数据导入", createdAt: new Date("2026-08-21T09:02:00+08:00").getTime() },
    ]],
  ]));
  const commentsRef = useRef<Map<string, string>>(new Map([
    [stableCellKey("bookcases-shanghai", "revenue"), "待复核：净收入与城市日报存在 2,400 元差异。"],
  ]));
  ...
  const [selectionStats, setSelectionStats] = useState<SelectionStats>(EMPTY_STATS);
  ...
  const [columnVisibility, setColumnVisibility] = useState(() => COLUMNS.map(() => true));
  ...
  const aggregateValue = getAggregateValue(selectionStats, aggregateMode, customFormula);
  ...
  useEffect(() => {
    ...
    const closePopoversOnOutsidePress = (event: PointerEvent) => {
      ...
      if (target instanceof Element && target.closest(".toolbar-popover-anchor")) return;
      setSearchOpen(false);
      setColumnMenuOpen(false);
    };
  ...
  useEffect(() => {
    ...
    let validationFlashCell: { row: number; col: number; backColor: string | null } | null = null;

    const notify = (message: string, tone: ToastTone = "success") => {
      window.clearTimeout(toastTimer);
      setToast({ message, tone });
      toastTimer = window.setTimeout(() => setToast(null), tone === "error" ? 3200 : 2400);
    };
    ...
    const start = async () => {
      ...
      const decimalValidator = GC.Spread.Sheets.DataValidation.createNumberValidator(
      ...
      decimalValidator.errorStyle(GC.Spread.Sheets.DataValidation.ErrorStyle.stop);

      const updateSelected = (row: number, col: number) => {
        const node = activeRows[row];
        const column = COLUMNS[col];
        if (!node || !column || col < 0 || col >= sheet.getColumnCount()) return;
        ...
        setSelected(cell);
        if (panelRef.current === "history") setSelectedHistory(historyRef.current.get(cell.key) ?? []);
      };

      const calculateSelection = (worksheet: Worksheet, range: import("@grapecity-software/spread-sheets").Spread.Sheets.Range) => {
        const startRow = Math.max(range.row, 0);
        const startCol = Math.max(range.col, 0);
        const rowCount = range.row < 0 ? worksheet.getRowCount() : range.rowCount;
        const colCount = range.col < 0 ? worksheet.getColumnCount() : range.colCount;
        ...
        for (let offset = 0; offset < inspectCount; offset += 1) {
          ...
          const stressDataLoaded = loadedStressPages.has(Math.floor(row / STRESS_PAGE_SIZE))
            || loadedStressRows.has(row);
          const value = activeDataMode === "stress" && !stressDataLoaded
          ...
          if (typeof value === "number" && Number.isFinite(value)) {
            ...
            numericDisplay = numericDisplay === null
              ? cellDisplay
              : numericDisplay === cellDisplay ? numericDisplay : "mixed";
            numeric += 1;
            sum += value;
      ...
      const clearOutlinesAndComments = () => {
        ...
        try { sheet.comments.clear(new GC.Spread.Sheets.Range(0, 0, oldRows, oldCols)); } catch { /* no comments */ }
      };

      const applyStableComments = () => {
        activeRows.forEach((row, rowIndex) => {
          COLUMNS.forEach((column, colIndex) => {
            const text = commentsRef.current.get(stableCellKey(row.id, column.field));
            if (text) sheet.comments.add(rowIndex, colIndex, text);
          });
        });
      };

      const styleStatusCells = (startRow = 0, rowCount = Math.min(activeRows.length, STRESS_PAGE_SIZE)) => {
        ...
        for (let row = startRow; row < endRow; row += 1) {
          ...
          if (status === "已核验") cell.backColor("#e6f5ef").foreColor("#19715d");
          else if (status === "异常") cell.backColor("#fee9e8").foreColor("#b13b3b");
      ...
      const syncGroupToolbarState = (isRowGroup: boolean) => {
        const collapsed = isRowGroup
          ? activeRowOutlineGroups.some(({ detailStart }) => sheet.rowOutlines.isCollapsed(detailStart))
          : COLUMN_GROUPS.some(({ detailStart }) => sheet.columnOutlines.isCollapsed(detailStart));
        if (isRowGroup) {
      ...
      const configureCellTypes = (startRow: number, rowCount: number) => {
        sheet.getRange(startRow, STATUS_COLUMN, rowCount, 1).cellType(statusCellType);
        sheet.getRange(startRow, VERIFIED_COLUMN, rowCount, 1).cellType(verifiedCellType);

        const updatedAtRange = sheet.getRange(startRow, UPDATED_AT_COLUMN, rowCount, 1);
        ...
        sheet.getRange(startRow, ATTACHMENT_COLUMN, rowCount, 1).cellType(attachmentCellType);
      ...
      const styleDataRows = (startRow: number, rowCount: number, columnCount: number) => {
        sheet.getRange(startRow, REVENUE_COLUMN, rowCount, 3).formatter("¥#,##0");
        sheet.getRange(startRow, ORDERS_COLUMN, rowCount, 3).formatter("#,##0");
        sheet.getRange(startRow, AVG_ORDER_COLUMN, rowCount, 1).formatter("¥#,##0");
        sheet.getRange(startRow, COMPLETION_COLUMN, rowCount, 1).formatter("0.0%");
        sheet.getRange(startRow, UPDATED_AT_COLUMN, rowCount, 1).formatter("yyyy-mm-dd");
        sheet.getRange(startRow, DECIMAL_COLUMN, rowCount, 1).formatter("0.00");
        sheet.getRange(startRow, 0, rowCount, columnCount).font("12px Arial, PingFang SC");
        sheet.getRange(startRow, 0, rowCount, 2)
          .backColor("#f4f6f8")
          .foreColor("#475467")
        ...
        sheet.getRange(startRow, 0, rowCount, columnCount).vAlign(GC.Spread.Sheets.VerticalAlign.center);
        const endRow = Math.min(activeRows.length, startRow + rowCount);
        for (let row = startRow; row < endRow; row += 1) {
          const hierarchyCol = hierarchyColumnForRow(activeRows[row]);
          sheet.getCell(row, hierarchyCol).textIndent(activeRows[row].hierarchyRole === "detail" ? 1 : 0);
        }
      };

      const refreshHierarchyNavigator = (startRow = 0, rowCount = activeRows.length) => {
        const endRow = Math.min(activeRows.length, startRow + rowCount);
        spread.suspendEvent();
        try {
          activeRowOutlineGroups.forEach(({ summaryRow, detailStart }) => {
            if (summaryRow < startRow || summaryRow >= endRow) return;
            const text = hierarchyCellText(activeRows[summaryRow], sheet.rowOutlines.isCollapsed(detailStart));
      ...
      const writeStressPage = (pageIndex: number) => {
        if (activeDataMode !== "stress" || loadedStressPages.has(pageIndex)) return;
        const startRow = pageIndex * STRESS_PAGE_SIZE;
        if (startRow < 0 || startRow >= activeRows.length) return;
        const rowCount = Math.min(STRESS_PAGE_SIZE, activeRows.length - startRow);
        let segmentStart = 0;
        for (let localRow = 0; localRow <= rowCount; localRow += 1) {
          const preserveExistingRow = localRow === rowCount || loadedStressRows.has(startRow + localRow);
        ...
        for (let row = startRow; row < startRow + rowCount; row += 1) loadedStressRows.delete(row);
      };

      const writeStressRow = (row: number) => {
        const page = Math.floor(row / STRESS_PAGE_SIZE);
        if (row < 0 || row >= activeRows.length || loadedStressPages.has(page) || loadedStressRows.has(row)) return;
        sheet.setArray(row, 0, [viewRowValues(activeRows[row], sheet.getColumnCount())]);
      ...
      const loadStressData = (pageIndices: Iterable<number>, rowIndices: Iterable<number> = []) => {
        if (activeDataMode !== "stress") return;
        const maxPage = Math.ceil(activeRows.length / STRESS_PAGE_SIZE) - 1;
        const unloadedPages = [...new Set(pageIndices)]
          .filter((page) => page >= 0 && page <= maxPage && !loadedStressPages.has(page));
        const pagesBeingLoaded = new Set(unloadedPages);
        const unloadedRows = [...new Set(rowIndices)]
          .filter((row) => row >= 0
      ...
      const loadVisibleStressRows = (fallbackTopRow = 0) => {
        ...
        const topRow = viewportTop >= 0 ? viewportTop : Math.max(fallbackTopRow, 0);
        ...
        if (!visibleRowsByPage.size) {
          visibleRowsByPage.set(Math.floor(topRow / STRESS_PAGE_SIZE), [topRow]);
        }

        const fullPages: number[] = [];
        const sparseRows: number[] = [];
        visibleRowsByPage.forEach((rows, page) => {
          if (rows.length >= STRESS_FULL_PAGE_VISIBLE_ROWS) fullPages.push(page);
        ...
        if (bottomRow - topRow < STRESS_PAGE_SIZE * 2
      ...
      const normalizeActiveSelection = () => {
        ...
        if (activeRow >= 0 && !sheet.getRowVisible(activeRow, GC.Spread.Sheets.SheetArea.viewport)) {
          for (nextRow = activeRow - 1; nextRow >= 0; nextRow -= 1) {
            if (sheet.getRowVisible(nextRow, GC.Spread.Sheets.SheetArea.viewport)) break;
          }
          if (nextRow < 0) nextRow = 0;
        }
        if (activeCol >= 0 && !sheet.getColumnVisible(activeCol, GC.Spread.Sheets.SheetArea.viewport)) {
          for (nextCol = activeCol - 1; nextCol >= 0; nextCol -= 1) {
            if (sheet.getColumnVisible(nextCol, GC.Spread.Sheets.SheetArea.viewport)) break;
          }
        ...
        sheet.showCell(nextRow, nextCol, GC.Spread.Sheets.VerticalPosition.nearest, GC.Spread.Sheets.HorizontalPosition.nearest);
        updateSelected(nextRow, nextCol);
        calculateSelection(sheet, new GC.Spread.Sheets.Range(nextRow, nextCol, 1, 1));
      };
      ...
      const renderRows = (rows: ViewRow[], stress: boolean) => {
        ...
        if (stress) {
          ...
          sheet.setArray(0, 0, rows.map((row) => viewRowValues(row, colCount)));
        }
        ...
        COLUMN_HEADER_GROUPS.forEach(({ label, startCol, colCount: groupColumnCount }) => {
          sheet.setValue(1, startCol, label, headerArea);
          if (groupColumnCount > 1) sheet.addSpan(1, startCol, 1, groupColumnCount, headerArea);
        });

        COLUMN_HEADER_SECTIONS.forEach(({ label, startCol, colCount: sectionColumnCount }) => {
          sheet.setValue(0, startCol, label, headerArea);
          if (sectionColumnCount > 1) sheet.addSpan(0, startCol, 1, sectionColumnCount, headerArea);
        });
        COLUMNS.forEach((column, col) => sheet.setColumnWidth(col, column.width));

        if (!stress) styleDataRows(0, rowCount, colCount);
        sheet.getRange(0, 0, 3, colCount, headerArea)
          .backColor("#f4f6fa")
          .foreColor("#344054")
          .hAlign(GC.Spread.Sheets.HorizontalAlign.center)
          .font("600 12px Arial, PingFang SC");
        sheet.getRange(1, 0, 1, colCount, headerArea)
          .backColor("#f7f5ff")
          .foreColor("#67569e")
          .font("650 12px Arial, PingFang SC");
        sheet.getRange(0, 0, 1, colCount, headerArea)
          .backColor("#e9e3fb")
          .foreColor("#513b9d")
        ...
        activeRowOutlineGroups.forEach(({ summaryRow, detailStart, detailCount }) => {
          sheet.rowOutlines.group(detailStart, detailCount);
          sheet.getRange(summaryRow, 0, 1, colCount).font("600 12px Arial, PingFang SC");
        });
        ...
        COLUMNS.forEach((_, col) => sheet.setColumnVisible(col, columnVisibilityRef.current[col]));
        } finally {
          spread.resumeEvent();
          sheet.resumeDirty();
          sheet.resumeCalcService(false);
        ...
        setDatasetLabel(`${rowCount.toLocaleString("zh-CN")} 行 × ${colCount} 列`);
      ...
      const openPanelForSelection = (nextPanel: Exclude<PanelName, null>) => {
        ...
        if (node && column) {
          ...
          if (nextPanel === "history") setSelectedHistory(historyRef.current.get(key) ?? []);
        }
        setSearchOpen(false);
        setColumnMenuOpen(false);
        setPanel(nextPanel);
      ...
      const setViewAndRender = (nextView: DrillView) => {
        ...
        const nextRows = activeDataMode === "stress"
      ...
      const toggleHierarchyRow = (row: number) => {
        const node = activeRows[row];
        const group = activeRowOutlineGroups.find((candidate) => candidate.summaryRow === row);
        ...
        runOutlineBatch(true, () => sheet.rowOutlines.expandGroup(groupInfo, !collapse));
        notify(collapse ? `已收起${node.name}的下级` : `已展开${node.name}的下级`);
      };
      ...
      const searchCellBlock = (query: string, rowStart: number, colStart: number, rowEnd: number, colEnd: number) => {
        ...
        condition.searchFlags = GC.Spread.Sheets.Search.SearchFlags.ignoreCase
          | GC.Spread.Sheets.Search.SearchFlags.blockRange;
        const result = sheet.search(condition);
        if (result.searchFoundFlag === GC.Spread.Sheets.Search.SearchFoundFlags.none) return null;
        return { row: result.foundRowIndex, col: result.foundColumnIndex };
      };

      const findLastCellMatch = (query: string, rowStart: number, colStart: number, rowEnd: number, colEnd: number) => {
        if (!searchCellBlock(query, rowStart, colStart, rowEnd, colEnd)) return null;

        let firstRow = rowStart;
        let lastRow = rowEnd;
        while (firstRow < lastRow) {
          const middleRow = Math.floor((firstRow + lastRow + 1) / 2);
          if (searchCellBlock(query, middleRow, colStart, rowEnd, colEnd)) firstRow = middleRow;
        ...
        while (firstCol < lastCol) {
          ...
          if (searchCellBlock(query, firstRow, middleCol, firstRow, colEnd)) firstCol = middleCol;
          else lastCol = middleCol - 1;
        }
        return searchCellBlock(query, firstRow, firstCol, firstRow, firstCol);
      };

      const findStressCellMatch = async (query: string, direction: 1 | -1, queryChanged: boolean, searchRun: number) => {
        ...
        for (let inspected = 0; inspected < totalCells; inspected += 1) {
          ...
          if (!textOnlyQuery || STRESS_TEXT_SEARCH_COLUMNS.has(col)) {
            ...
            const text = stressDataLoaded
              ? `${sheet.getText(row, col)} ${sheet.getFormula(row, col) ?? ""} ${sheet.getValue(row, col) ?? ""}`.toLowerCase()
              : stressCellSearchText(activeRows[row], col, includeFormattedNumber).toLowerCase();
      ...
      const revealSearchMatch = (row: number, col: number) => {
        if (!sheet.getRowVisible(row)) {
          runOutlineBatch(true, () => {
            for (let level = 0; level <= sheet.rowOutlines.getMaxLevel(); level += 1) {
        ...
        if (!sheet.getColumnVisible(col)) {
          runOutlineBatch(false, () => {
            for (let level = 0; level <= sheet.columnOutlines.getMaxLevel(); level += 1) {
          ...
          if (!sheet.getColumnVisible(col)) {
            ...
            if (col < COLUMNS.length) {
              ...
              setColumnVisibility((current) => current.map((value, index) => index === col ? true : value));
            }
          }
        }
        sheet.setActiveCell(row, col);
        sheet.setSelection(row, col, 1, 1);
        sheet.showCell(row, col, GC.Spread.Sheets.VerticalPosition.center, GC.Spread.Sheets.HorizontalPosition.center);
        updateSelected(row, col);
      ...
      const search = async (query: string, direction: 1 | -1) => {
        ...
        const queryChanged = activeSearch.query !== trimmed || activeSearch.row < 0 || activeSearch.col < 0;
        if (activeDataMode === "stress") {
          const searchRun = ++activeSearchRun;
          setSearchResult("正在搜索 10 万行…");
          const stressMatch = await findStressCellMatch(trimmed, direction, queryChanged, searchRun);
          ...
          activeSearch = { query: trimmed, row: stressMatch.row, col: stressMatch.col };
          ensureStressRowLoaded(stressMatch.row);
          revealSearchMatch(stressMatch.row, stressMatch.col);
          setSearchResult(`匹配于 ${columnName(stressMatch.col)}${stressMatch.row + 1}`);
          return;
        }
        ...
        const match = direction === 1
          ? searchCellBlock(trimmed, cursor.row, cursor.col + 1, cursor.row, lastCol)
          ...
          : findLastCellMatch(trimmed, cursor.row, 0, cursor.row, cursor.col - 1)
            ?? findLastCellMatch(trimmed, 0, 0, cursor.row - 1, lastCol)
            ?? findLastCellMatch(trimmed, cursor.row + 1, 0, lastRow, lastCol)
            ?? findLastCellMatch(trimmed, cursor.row, cursor.col, cursor.row, lastCol);
      ...
      actionsRef.current = {
        ...
        redo: () => {
          spread.commandManager().execute({ cmd: "redo", sheetName: sheet.name() });
          notify("已重做上一次单元格操作");
        },
        copy: () => {
        ...
        toggleColumn: (col, visible) => {
          ...
          setColumnVisibility((current) => current.map((value, index) => index === col ? visible : value));
        ...
        toggleRowGroups: () => {
          ...
          runOutlineBatch(true, () => {
            if (collapse) {
              for (let level = sheet.rowOutlines.getMaxLevel(); level >= 0; level -= 1) {
        ...
        toggleColumnGroups: () => {
          ...
          runOutlineBatch(false, () => {
            if (collapse) {
              for (let level = sheet.columnOutlines.getMaxLevel(); level >= 0; level -= 1) {
          ...
          stressLoadTimer = window.setTimeout(() => {
            ...
            void getStressRecordsAsync()
              .then((rows) => {
                ...
                setDataMode("stress");
                notify(`10 万行已载入，用时 ${Math.round(performance.now() - startedAt)} ms`);
              })
              .catch((error: unknown) => {
        ...
        focusAttachment: () => {
          ...
          sheet.showCell(row, ATTACHMENT_COLUMN, GC.Spread.Sheets.VerticalPosition.center, GC.Spread.Sheets.HorizontalPosition.center);
          updateSelected(row, ATTACHMENT_COLUMN);
          calculateSelection(sheet, new GC.Spread.Sheets.Range(row, ATTACHMENT_COLUMN, 1, 1));
          setPanel(null);
          notify("已定位到附件单元格，点击上传按钮选择文件");
        },
      };

      spread.commandManager().register("businessComment", { canUndo: false, execute: () => { openPanelForSelection("comment"); return true; } });
      spread.commandManager().register("businessHistory", { canUndo: false, execute: () => { openPanelForSelection("history"); return true; } });
      spread.commandManager().register("businessLineage", { canUndo: false, execute: () => { openPanelForSelection("lineage"); return true; } });
      spread.commandManager().register("businessAttachment", { canUndo: false, execute: () => { openPanelForSelection("attachment"); return true; } });
      spread.commandManager().register("businessDrill", { canUndo: false, execute: () => { drillSelected(); return true; } });
      spread.contextMenu.menuData.push(
        { text: "新增 / 编辑批注", name: "business-comment", command: "businessComment", workArea: "viewport" },
        { text: "查看值历史", name: "business-history", command: "businessHistory", workArea: "viewport" },
        { text: "数据追踪", name: "business-lineage", command: "businessLineage", workArea: "viewport" },
        { text: "附件管理", name: "business-attachment", command: "businessAttachment", workArea: "viewport" },
        { text: "下钻到下一层", name: "business-drill", command: "businessDrill", workArea: "viewport" },
      );

      sheet.bind(GC.Spread.Sheets.Events.EnterCell, (_sender: unknown, args: { row: number; col: number }) => {
        updateSelected(args.row, args.col);
      });
      sheet.bind(GC.Spread.Sheets.Events.SelectionChanged, (_sender: unknown, args: SelectionChangedArgs) => {
        const range = args.newSelections.at(-1);
        if (range) calculateSelection(sheet, range);
      });
      sheet.bind(GC.Spread.Sheets.Events.CellClick, (_sender: unknown, args: CellClickArgs) => {
      ...
      sheet.bind(GC.Spread.Sheets.Events.CellDoubleClick, (_sender: unknown, args: CellDoubleClickArgs) => {
        if (args.sheetArea !== GC.Spread.Sheets.SheetArea.viewport) return;
        const node = activeRows[args.row];
        if (!node || args.col === hierarchyColumnForRow(node)) return;
        drillRow(args.row);
      });
      spread.bind(GC.Spread.Sheets.Events.ClipboardChanged, (_sender: unknown, args: ClipboardChangedArgs) => {
      ...
      spread.bind(GC.Spread.Sheets.Events.ClipboardPasting, (_sender: unknown, args: ClipboardPastingArgs) => {
      ...
      sheet.bind(GC.Spread.Sheets.Events.ValidationError, (_sender: unknown, args: ValidationErrorArgs) => {
        ...
        if (validationFlashCell) {
          sheet.getCell(validationFlashCell.row, validationFlashCell.col)
      ...
      sheet.bind(GC.Spread.Sheets.Events.CellChanged, (_sender: unknown, args: CellChangedArgs) => {
        if (args.propertyName !== "value" || args.row < 0 || args.col < 0) return;
        const node = activeRows[args.row];
        ...
        if (isHierarchyField(column.field)) {
          spread.suspendEvent();
          try {
            const group = activeRowOutlineGroups.find((candidate) => candidate.summaryRow === args.row);
            const hierarchyCol = hierarchyColumnForRow(node);
            sheet.setValue(
              args.row,
              args.col,
              args.col === hierarchyCol
                ? hierarchyCellText(node, group ? sheet.rowOutlines.isCollapsed(group.detailStart) : false)
                : null,
            );
          } finally { spread.resumeEvent(); }
          notify("层级字段用于展开和折叠，名称请从业务数据源维护");
          updateSelected(args.row, args.col);
          return;
        }
        let nextValue = args.newValue;
        if (column.field === "adjustmentFactor" && typeof nextValue === "number") {
          const rounded = roundToTwoDecimals(nextValue);
          if (rounded !== nextValue) {
            spread.suspendEvent();
            try { sheet.setValue(args.row, args.col, rounded); } finally { spread.resumeEvent(); }
            nextValue = rounded;
          }
        }
        updateBusinessNode(node, column.field, nextValue);
        const sourceNode = activeDataMode === "stress"
          ? stressSourceRows?.find((row) => row.id === node.id)
          : findBusinessNode(BUSINESS_DATA, node.id);
        if (sourceNode && sourceNode !== node) updateBusinessNode(sourceNode, column.field, nextValue);
        ...
        historyRef.current.set(key, [nextItem, ...(historyRef.current.get(key) ?? [])].slice(0, 30));
        updateSelected(args.row, args.col);
      });
      sheet.bind(GC.Spread.Sheets.Events.RangeGroupStateChanged, (_sender: unknown, args: RangeGroupStateChangedArgs) => {
        if (!groupChangeBatching) refreshAfterGroupChange(args.isRowGroup);
      });
      sheet.bind(GC.Spread.Sheets.Events.TopRowChanged, (_sender: unknown, args: TopRowChangedArgs) => {
        if (activeDataMode === "stress") scheduleStressViewportLoad(args.newTopRow);
      });
  ...
  const openPanel = (nextPanel: Exclude<PanelName, null>) => actionsRef.current?.openPanel(nextPanel);
